...
使用 Rust 编写，速度极快，测试完善。同时也是 [CC之友（CC Friend / Claude Code Friend）](https://github.com/cc-friend/ccfriend)（一个 Tauri 桌面应用）的底层库。
...
[English Docs](https://github.com/cc-friend/ccost#readme)
...
或从 [GitHub Releases](https://github.com/cc-friend/ccost/releases) 下载预编译二进制文件。
...
| 参数 | 说明 |
|------|------|
| `--per <dim>` | 分组维度：`day`、`hour`、`month`、`session`、`project`、`model`、`subagent`、`tool`、`line`。最多指定2个做嵌套分组。默认 `--per day --per model`。 |
...
# 工具费用分析：哪些工具消耗最多 token
ccost --per tool --cost decimal

# 按 JSONL 行分析：每行的 token 开销
...
每条记录的 `message.content` 使用轻量级反序列化解析（仅提取内容块中的 `type` 和 `name` 字段；`input`、`text` 等字段被跳过以保证性能）。从 assistant 消息的 `tool_use` 块中提取工具名称，支持 `--per tool` 分组。每条记录还携带其 JSONL 行号（从 1 开始），支持 `--per line` 分组。行号与 ccfriend 的转录行锚点（`#N`）一致。
...
`statusline.jsonl` 的核心价值在于速率限制百分比，而非费用。因此 [CC之友（CC Friend / Claude Code Friend）](https://github.com/cc-friend/ccfriend) 使用的是从 `~/.claude/projects/` 计算的费用数据，结合 `statusline.jsonl` 中的5小时和1周速率限制百分比数据。
...
![终端输出](https://raw.githubusercontent.com/cc-friend/ccost/main/screenshot_terminal.png)

### 状态栏分析（`ccost sl`）

![状态栏输出](https://raw.githubusercontent.com/cc-friend/ccost/main/screenshot_terminal_sl.png)

### HTML 报告

![HTML 报告](https://raw.githubusercontent.com/cc-friend/ccost/main/screenshot_html.png)
