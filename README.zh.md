...
使用 Rust 编写，速度极快，测试完善。同时也是 [CC仪表盘（Claude Code 仪表盘）](https://github.com/cc-friend/ccfriend)（一个 Tauri 桌面应用）的底层库。
...
`statusline.jsonl` 的核心价值在于速率限制百分比，而非费用。因此 [Claude Code 仪表盘](https://github.com/cc-friend/ccfriend) 使用的是从 `~/.claude/projects/` 计算的费用数据，结合 `statusline.jsonl` 中的5小时和1周速率限制百分比数据。
